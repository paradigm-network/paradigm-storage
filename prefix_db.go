...
func IteratePrefix(db DB, prefix []byte) Iterator {
	...
	if len(prefix) == 0 {
		...
		start = common.Cp(prefix)
		end = common.CpIncr(prefix)
	}
...
func (pdb *prefixDB) Iterator(start, end []byte) Iterator {
	...
	pstart = append(common.Cp(pdb.prefix), start...)
	if end == nil {
		pend = common.CpIncr(pdb.prefix)
	} else {
		pend = append(common.Cp(pdb.prefix), end...)
	}
...
func (pdb *prefixDB) ReverseIterator(start, end []byte) Iterator {
	...
	if start == nil {
		...
		pstart = common.CpIncr(pdb.prefix)
	} else {
		pstart = append(common.Cp(pdb.prefix), start...)
	}
	if end == nil {
		...
		pend = common.CpDecr(pdb.prefix)
	} else {
		pend = append(common.Cp(pdb.prefix), end...)
	}
	ritr := pdb.db.ReverseIterator(pstart, pend)
	if start == nil {
		skipOne(ritr, common.CpIncr(pdb.prefix))
	}
	return newPrefixIterator(
		pdb.prefix,
		start,
...
func (pdb *prefixDB) prefixed(key []byte) []byte {
	return append(common.Cp(pdb.prefix), key...)
}
...
func (pb prefixBatch) Set(key, value []byte) {
	pkey := append(common.Cp(pb.prefix), key...)
