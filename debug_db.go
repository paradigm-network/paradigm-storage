package paradigm_storage

import (
	"fmt"
	"sync"
	"github.com/paragidm-network/paradigm-storage/common"
)
...
func (ddb debugDB) Get(key []byte) (value []byte) {
	defer func() {
		fmt.Printf("%v.Get(%v) %v\n", ddb.label, common.Cyan(_fmt("%X", key)), common.Blue(_fmt("%X", value)))
	}()
...
func (ddb debugDB) Has(key []byte) (has bool) {
	defer func() {
		fmt.Printf("%v.Has(%v) %v\n", ddb.label, common.Cyan(_fmt("%X", key)), has)
	}()
...
func (ddb debugDB) Set(key []byte, value []byte) {
	fmt.Printf("%v.Set(%v, %v)\n", ddb.label, common.Cyan(_fmt("%X", key)), common.Yellow(_fmt("%X", value)))
...
func (ddb debugDB) Delete(key []byte) {
	fmt.Printf("%v.Delete(%v)\n", ddb.label, common.Red(_fmt("%X", key)))
	ddb.db.Delete(key)
}
...
func (ddb debugDB) Iterator(start, end []byte) Iterator {
	fmt.Printf("%v.Iterator(%v, %v)\n", ddb.label, common.Cyan(_fmt("%X", start)), common.Blue(_fmt("%X", end)))
...
func (ditr debugIterator) Key() (key []byte) {
	fmt.Printf("%v.itr.Key() %v\n", ditr.label, common.Cyan(_fmt("%X", key)))
	key = ditr.itr.Key()
	return
}
...
func (ditr debugIterator) Value() (value []byte) {
	fmt.Printf("%v.itr.Value() %v\n", ditr.label, common.Blue(_fmt("%X", value)))
